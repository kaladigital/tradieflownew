...
//    $RCMAIL->output->send($plugin['task']);
    try{
        $dbh = new PDO('mysql:host='.$RCMAIL->config->get('mysql_td_db_host').';dbname='.$RCMAIL->config->get('mysql_td_db_name'), $RCMAIL->config->get('mysql_td_db_user'), $RCMAIL->config->get('mysql_td_db_password'));
//var_dump('mysql:host='.$RCMAIL->config->get('mysql_td_db_host').';dbname='.$RCMAIL->config->get('mysql_td_db_name'), $RCMAIL->config->get('mysql_td_db_user'), $RCMAIL->config->get('mysql_td_db_password'));die;
        $stmt = $dbh->prepare("SELECT * FROM user WHERE gmail_token=? LIMIT 1");
        $stmt->execute([$_GET['token']]);
        $row = $stmt->fetch();

        if ($row['gmail_username'] && strlen($row['gmail_password'])) {
            $request_valid = $_SESSION['temp'] && $RCMAIL->check_request();
            $pass_charset = $RCMAIL->config->get('password_charset', 'UTF-8');
            ...
            $auth = $RCMAIL->plugins->exec_hook('authenticate', [
                'host' => $RCMAIL->autoselect_host(),
                'user' => $row['gmail_username'],
                'pass' => $row['gmail_password'],
                'valid' => $request_valid,
                'error' => null,
                'cookiecheck' => true,
            ]);
...
//            // Login
        ...
        exit('Failed to connect to mailing server, please refresh page or contact support '.$e->getMessage());
    }
}
else {
    // CSRF prevention
